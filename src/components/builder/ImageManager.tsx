      ...
      className={`
        ...
        ${
          isDragging
            ...
            : 'border-gray-300 bg-muted/30 hover:border-emerald-400 hover:bg-muted/50'
        }
...
export function ImageManager({
  ...
  const handleDeleteImage = useCallback(
    async (url: string) => {
      // Try to delete from server (only for uploaded files, not external URLs)
      if (url.startsWith('/uploads/')) {
        try {
          ...
          // Server delete failed — still remove from local list
          console.warn('Could not delete file from server:', url);
        }
      }
          ...
          {value ? (
            <>
              ...
              <div className="min-w-0 flex-1 text-left">
                ...
                <p className="truncate text-xs text-gray-400">Imagen seleccionada</p>
              </div>
              <X
                className="h-4 w-4 shrink-0 text-gray-400 hover:text-red-500"
                onClick={(e) => {
                  e.stopPropagation();
