...
function CollapsibleGroup({
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-gray-50/50 overflow-hidden">
      <button
        type="button"
        className="flex w-full items-center gap-2 px-3 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-100/80 transition-colors"
      ...
      {open && (
        <div className="border-t border-gray-200 bg-white px-3 py-3 space-y-3.5">
          {children}
        </div>
      )}
    </div>
...
function SubHeader({ children }: { children: React.ReactNode }) {
  return (
    <p className="text-[10px] text-gray-400 uppercase tracking-wider font-semibold">{children}</p>
  );
}
